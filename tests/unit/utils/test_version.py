...
# The imports below pull in the qutebrowser.qt layer which requires PyQt.
# In CI environments PyQt may not be available for all test environments
# (see fault/CI logs). Import lazily and fall back to None on ImportError so
# tests that don't require PyQt can still be collected and run.
try:
...
class TestWebEngineVersions:
    ...
    def test_chromium_security_version_dict(self, qapp):
        ...
        # The reported "security patch" (last component) can differ between
        # environments; ensure major/minor/build components match instead of
        # requiring strict equality of the patch-level component.
