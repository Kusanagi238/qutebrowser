...
try:
    ...
    # Qt bindings are not available in this environment. Skip the test module
    # rather than raising ImportError during import-time to allow other test
    # environments to run without requiring PyQt5/PySide.
    machinery = None
...
class TestWebEngineVersions:
    ...
    def test_chromium_security_version_dict(self, qapp):
        ...
        actual = qWebEngineChromiumSecurityPatchVersion()
        ...
        # Accept the inference if it is at least as recent as the API-provided
        # security patch version. This avoids brittle failures when the
        # mapping in version.py is updated independently of the system SDK.
        assert _parse(inferred.chromium_security) >= _parse(
            actual
        ), f"inferred {inferred.chromium_security!r} is older than actual {actual!r}"
